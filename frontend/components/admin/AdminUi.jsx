...
export function AdminSurface({ className = "", children }) {
  return <div className={cn("admin-content-surface rounded-[28px] border border-slate-100 bg-white shadow-[0_18px_44px_rgba(15,23,42,0.05)]", className)}>{children}</div>;
}
...
export function AdminStatusChip({ status }) {
  ...
  return <span className={cn("inline-flex rounded-full px-3 py-1 text-xs font-semibold capitalize", styles[normalized] || "bg-slate-100 text-slate-500")}>{normalized.replaceAll("_", " ")}</span>;
}
    ...
    <AdminSurface className="p-4">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center">
        <div className="admin-input-surface flex min-w-0 flex-1 items-center gap-3 rounded-2xl border border-slate-200 bg-white px-4 py-3">
          ...
          <input value={searchValue} onChange={(event) => onSearchChange?.(event.target.value)} placeholder={searchPlaceholder} className="min-w-0 flex-1 bg-transparent text-sm font-medium text-slate-700 outline-none placeholder:text-slate-400" />
        </div>
        {filters.map((filter) => (
...
export function AdminPrimaryButton({ className = "", children, ...props }) {
  return <button {...props} className={cn("inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-[#16a34a] to-[#22c55e] px-5 py-3 text-sm font-bold text-white shadow-[0_12px_24px_rgba(34,197,94,0.22)]", className)}>{children}</button>;
}

export function AdminGhostButton({ className = "", children, ...props }) {
  return <button {...props} className={cn("admin-input-surface inline-flex items-center justify-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700", className)}>{children}</button>;
}
...
export function AdminPagination({ totalCount = 0, page = 1, pageSize = 10, onPageChange, onPageSizeChange, pageSizeOptions = [10, 20, 50] }) {
  ...
  return (
    <div className="flex flex-col gap-3 border-t border-slate-100 pt-4 text-sm text-slate-400 lg:flex-row lg:items-center lg:justify-between">
      <p>Showing {start} to {end} of {formatCompactNumber(totalCount)} records</p>
      <div className="flex flex-wrap items-center gap-2">
        <button type="button" onClick={() => onPageChange?.(Math.max(1, page - 1))} disabled={page <= 1} className="rounded-xl border border-slate-200 bg-white px-3 py-2 font-semibold text-slate-600 disabled:cursor-not-allowed disabled:opacity-50">Prev</button>
        {visiblePages.map((pageNumber) => (
          <button key={pageNumber} type="button" onClick={() => onPageChange?.(pageNumber)} className={`h-10 min-w-[40px] rounded-xl border px-3 font-bold ${pageNumber === page ? "border-emerald-500 bg-emerald-500 text-white" : "border-slate-200 bg-white text-slate-600"}`}>{pageNumber}</button>
        ))}
        <button type="button" onClick={() => onPageChange?.(Math.min(totalPages, page + 1))} disabled={page >= totalPages} className="rounded-xl border border-slate-200 bg-white px-3 py-2 font-semibold text-slate-600 disabled:cursor-not-allowed disabled:opacity-50">Next</button>

            ...
            <div className="space-y-2">
              {sections.map((section) => (
                <button
                  ...
                  className={`flex w-full items-center gap-3 rounded-2xl px-4 py-3 text-left text-sm font-semibold ${activeSection === section ? "bg-[#f4efff] text-[#6f4cff]" : "text-slate-600 hover:bg-slate-50"}`}
                >
                  <span className={`h-2 w-2 rounded-full ${activeSection === section ? "bg-[#6f4cff]" : "bg-slate-300"}`} />
                  {section}
                </button>
              ))}
            </div>
              ...
              <div className="mt-5 space-y-4">
                {[
                  ...
                  <label key={key} className="flex items-center justify-between gap-4 rounded-2xl border border-slate-100 bg-slate-50 px-4 py-3">
                    ...
                    <input type="checkbox" checked={form[key]} onChange={(event) => setForm((current) => ({ ...current, [key]: event.target.checked }))} className="h-5 w-5 accent-[#6f4cff]" />
                  </label>
                ))}
              </div>
            </AdminSurface>

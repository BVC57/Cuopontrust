...
export default function AdminWithdrawalsPage() {
  ...
  const byMethod = useMemo(() => {
    const grouped = withdrawals.reduce((acc, row) => {
      const key = row.upiId ? "UPI" : row.bankDetails ? "Bank Transfer" : "Others";
                ...
                <tbody>
                  {filtered.map((row, index) => (
                    <tr key={row._id} className={index ? "border-t border-slate-100" : ""}>
                      ...
                      <td className="px-5 py-4 text-sm font-semibold text-slate-600">{row.upiId ? "UPI" : "Bank Transfer"}</td>
                      <td className="px-5 py-4 text-sm font-medium text-slate-600">{row.upiId || row.bankDetails || "-"}</td>
        ...
        {selectedWithdrawal ? (
          <div className="space-y-4">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-bold uppercase tracking-[0.18em] text-slate-400">Method</p><p className="mt-2 text-sm font-bold text-slate-900">{selectedWithdrawal.upiId ? "UPI" : "Bank Transfer"}</p></div>

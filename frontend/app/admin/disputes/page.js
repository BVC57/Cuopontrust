                ...
                <tbody>
                  {disputes.map((row, index) => (
                    <tr key={row._id} className={index ? "border-t border-slate-100" : ""}>
                      ...
                      <td className="px-5 py-4 text-sm font-bold text-[#6f4cff]">{row.transactionId?.slice?.(-9)?.toUpperCase?.() || row.transactionId || "-"}</td>
        ...
        {selected ? (
          <div className="space-y-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="rounded-2xl bg-slate-50 p-4"><p className="text-xs font-black uppercase tracking-[0.18em] text-slate-400">Dispute ID</p><p className="mt-2 text-sm font-bold text-[#6f4cff]">{selected._id.slice(-9).toUpperCase()}</p></div>
